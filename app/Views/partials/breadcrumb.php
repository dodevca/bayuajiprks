                    ...
                    <?php foreach($meta->name as $key => $name): ?>
                        <li class="breadcrumb-item<?= $key == count($meta->name) - 1 ? ' active' : '' ?>" <?= $key == count($meta->name) - 1 ? 'aria-current="page"' : '' ?>>
                            <?php if($key != count($meta->name) - 1): ?>
                                <a href="<?= base_url($name) ?>" class="text-decoration-none"><?= $name ?></a>
                            <?php else: ?>
                                <h1 class="d-inline h6 fw-normal text-muted mb-0"><?= $name ?></h1>
                            <?php endif; ?>
                        </li>
                    <?php endforeach; ?>
                <?php else: ?>
                    <li class="breadcrumb-item active" aria-current="page"><h1 class="d-inline h6 fw-normal text-muted mb-0"><?= $meta->name ?></h1></li>
                <?php endif; ?>
            </ol>
        </nav>
    </div>
